...
class Curses(object):
    ...
    def draw_conditions(self, hero, index):
        ...
        idx = index
        if hero.is_wounded:
            self.hero_window.addstr(idx, 2, "Wounded", curses.color_pair(1))
            idx += 1
        if hero.is_stunned:
    ...
    def help(self):
        ...
        help_screen.addstr(row + 3, cmd_col, "w wield a weapon")
        help_screen.addstr(row + 4, cmd_col, "m message history")
    ...
    def score_screen(self, time_score, map_score, kill_score, victory_score):
        ...
        score_screen.addstr(4, 33, str(kill_score))
